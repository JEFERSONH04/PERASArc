...
export const downloadFile = async (analysisIdResult) => {
    const apiUrl = `http://localhost:8000/api/v1/results/${analysisIdResult}/download`;
    const accessToken = localStorage.getItem('access');
    ...
    try {
        const response = await axios.get(apiUrl, {
            ...
            responseType: 'blob', // Esto es crucial para descargar archivos
        });

        // Crea una URL para el blob y un enlace temporal para la descarga
        ...
        console.error(`Error al descargar el archivo para el ID ${analysisIdResult}:`, error);
        throw error;
    }
};
